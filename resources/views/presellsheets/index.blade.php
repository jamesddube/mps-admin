    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <ul class="nav nav-pills">
                <li class="active"><a href="#nav-pills-tab-1" data-toggle="tab">ALL sheetS</a></li>
            ...
            <div class="tab-content">
                <div class="tab-pane fade active in" id="nav-pills-tab-1">
                    <div class="m-20">
                        <table id="data-table" class="table table-striped table-bsheeted nowrap" width="100%">
                            <thead>
                            <tr>
                                <th>sheet Number</th>
                                <th>Customer</th>
                            ...
                            @foreach($sheets as $sheet)
                                <tr class="odd gradeX">
                                    <td data-toggle="collapse" data-target="#{{ $sheet->id }}">{{ $sheet->id }}</td>
                                    <td>{{ $sheet->customer }}</td>
                                    <td>{{ $sheet->sales_rep }}</td>
                                    <td><span class="label label-success">{{ $sheet->sheet_status }}</span></td>
                                    <td>Mutare</td>
                                    <td><a href="{{ url('sheets/'.$sheet->id) }}" class="btn btn-sm btn-inverse">
                                            <i class="fa fa-search pull-left"></i>
                                            View
                                        </a>
                                        <div v-on:click="processsheet('{{ $sheet->id }}')" class="btn btn-sm btn-success">
                                            Process
                                        </div>
                                ...
                                <tr id="{{ $sheet->id }}" class="table collapse">
                                    <td>d</td>
                                    <td>d</td>
                                    <td>d</td>
                                </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Process sheet</h4>
                </div>
    ...
    <script>
        new Vue({
            ...
            methods: {
                ...
                fetchsheet: function (sheet) {
                    $.ajax({
                        url: 'api/sheets/'+ sheet,
                        type: 'get',
                        dataType: 'json',
                        async: false,
                        success: function (data) {
